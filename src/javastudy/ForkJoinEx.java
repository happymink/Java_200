...
class ForkJoinEx {
    ...
    public static void main(String[] args) {
        long from = 1L, to = 100_000_000L; // (1 ~ 1억)까지

        SumTask task = new SumTask(from, to); //수행할 작업 생성

        long start = System.currentTimeMillis();	// 시작시간 초기화

        Long result = pool.invoke(task);  // 쓰레드가 run이 아니라 start를 호출하는 것 처럼 compute가 아닌 invoke로 작업 시작
...
class SumTask extends RecursiveTask<Long> {
    ...
    public Long compute() {
        ...
        leftSum.fork(); //작업 (left sum)을 작업 큐에 넣는다.  , 비동기 메서드 , 호출 후 결과를 기다리지 않는다.
